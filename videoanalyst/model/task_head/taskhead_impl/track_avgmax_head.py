...
@VOS_HEADS.register
class DenseboxAvgMaxHead(ModuleBase):
    r"""
    ...
    def __init__(self):
        super(DenseboxAvgMaxHead, self).__init__()
        ...
        self.cls_convs = []
        self.bbox_convs = []
        self.cls_pgl = nn.Parameter(torch.ones(1), True).to("cuda")
        self.ctr_pgl = nn.Parameter(torch.ones(1), True).to("cuda")
    ...
    def forward(self, c_out, r_out, x_size=0, raw_output=False):
        ...
        # classification average pooling and max pooling
        cls_avg = torch.mean(cls, 1).unsqueeze(1)
        cls_max = torch.max(cls, 1)[0].unsqueeze(1)
        cls_avgmax_output = cls_avg + cls_max
        # quality assessment average pooling and max pooling
        ctr_avg = torch.mean(cls, 1).unsqueeze(1)
        ctr_max = torch.max(cls, 1)[0].unsqueeze(1)
        ctr_avgmax_output = ctr_avg + ctr_max
        # classification score
        cls_score = self.cls_score_p5(cls)  #todo
        # classification merge avg, max with origin p5
        cls_score = self.cls_pgl * cls_score + (1 - self.cls_pgl) * cls_avgmax_output

        cls_score = cls_score.permute(0, 2, 3, 1)
        cls_score = cls_score.reshape(cls_score.shape[0], -1, 1)
        # center-ness score
        ctr_score = self.ctr_score_p5(cls)  #todo
        # regression merge avg, max with origin p5
        ctr_score = self.ctr_pgl * ctr_score + (1 - self.ctr_pgl) * ctr_avgmax_output
