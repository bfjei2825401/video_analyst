...
class SiamAttention2CorrTrack(ModuleBase):
    r"""
    ...
    def __init__(self, backbone, head, loss=None):
        super(SiamAttention2CorrTrack, self).__init__()
    ...
    def forward(self, *args, phase="train"):
        ...
        if phase == 'train':
            ...
            r_out = self.reg_attention1(r_out)
            r_out = self.reg_attention2(r_out)
            # correlation cls residual attention
            c_out = self.cls_attention1(c_out)
            c_out = self.cls_attention2(c_out)
            # head
    ...
    def _initialize_attention(self, ):
        conv_weight_std = self._hyper_params['conv_weight_std']
        attention_list = [
            self.cls_attention1, self.cls_attention2,
            self.reg_attention1, self.reg_attention2
        ]
    ...
    def _make_attention(self):
        ...
        self.cls_attention1 = ResidualAttentionLayer(head_width,
                                                     ...
                                                     down_num=down_num,
                                                     up_num=up_num)
        self.cls_attention2 = ResidualAttentionLayer(head_width,
                                                     down_k_size=down_k_size,
                                                     down_stride=down_stride,
                                                     up_k_size=up_k_size,
                                                     up_stride=up_stride,
                                                     down_num=down_num,
                                                     up_num=up_num)
        self.reg_attention1 = ResidualAttentionLayer(head_width,
                                                     down_k_size=down_k_size,
                                                     down_stride=down_stride,
                                                     up_k_size=up_k_size,
                                                     up_stride=up_stride,
                                                     down_num=down_num,
                                                     up_num=up_num)
        self.reg_attention2 = ResidualAttentionLayer(head_width,
                                                     down_k_size=down_k_size,
                                                     down_stride=down_stride,
                                                     up_k_size=up_k_size,
                                                     up_stride=up_stride,
