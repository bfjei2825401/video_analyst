...
class SiamAttentionCRTrack(ModuleBase):
    r"""
    ...
    def __init__(self, backbone, head, loss=None):
        super(SiamAttentionCRTrack, self).__init__()
    ...
    def forward(self, *args, phase="train"):
        ...
        if phase == 'train':
            ...
            c_x = self.c_x(f_x)
            r_x = self.r_x(f_x)
            # attention before correlation
            # attention for search image only
            c_x = self.c_attention(c_x)
            r_x = self.r_attention(r_x)
            # feature matching
