...
class SiamSKTrack(ModuleBase):
    ...
    def __init__(self, backbone, head, loss=None):
        super(SiamSKTrack, self).__init__()
    ...
    def forward(self, *args, phase="train"):
        ...
        if phase == 'train':
            ...
            # exemplar SK layer
            c_z_k = self.sk_conv_c_z_k(c_z_k)
            r_z_k = self.sk_conv_r_z_k(r_z_k)
            # search adjustment
            c_x = self.c_x(f_x)
            r_x = self.r_x(f_x)
            # search SK layer
    ...
    def _make_sk_conv(self):
        ...
        self.sk_conv_c_z_k = SKConv(head_width, M=kernel_num, r=discount)
        self.sk_conv_r_z_k = SKConv(head_width, M=kernel_num, r=discount)
        self.sk_conv_c_x = SKConv(head_width, M=kernel_num, r=discount)
        self.sk_conv_r_x = SKConv(head_width, M=kernel_num, r=discount)

    def _initialize_sk_conv(self):
        conv_weight_std = self._hyper_params['conv_weight_std']
        sk_conv_list = [
            self.sk_conv_c_z_k, self.sk_conv_r_z_k, self.sk_conv_c_x, self.sk_conv_r_x
        ]
