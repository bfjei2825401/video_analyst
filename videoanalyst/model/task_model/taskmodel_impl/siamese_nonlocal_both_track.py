...
from videoanalyst.model.common_opr.common_block import (conv_bn_relu,
                                                        xcorr_depthwise)
from videoanalyst.model.common_opr.nonlocal_block import NonLocalBlock
...
class SiamNonLocalBothTrack(ModuleBase):
    r"""
    ...
    def __init__(self, backbone, head, loss=None):
        super(SiamNonLocalBothTrack, self).__init__()
    ...
    def forward(self, *args, phase="train"):
        ...
        if phase == 'train':
            ...
            # nonlocal block for exemplar feature
            c_z_k = self.cls_z_nonlocal(c_z_k)
            r_z_k = self.reg_z_nonlocal(r_z_k)

            c_x = self.c_x(f_x)
            r_x = self.r_x(f_x)
            # nonlocal block for sample feature
            c_x = self.cls_x_nonlocal(c_x)
            r_x = self.reg_x_nonlocal(r_x)
            # feature matching
    ...
    def _initialize_nonlocal(self):
        nonlocal_list = [self.cls_z_nonlocal, self.reg_z_nonlocal, self.cls_x_nonlocal, self.reg_x_nonlocal]
        for ith in range(len(nonlocal_list)):
            nonlocal_block = nonlocal_list[ith]
            nonlocal_block.zero_initialize()

    def _make_nonlocal(self):
        head_width = self._hyper_params['head_width']
        inner_plane = self._hyper_params['nonlocal_inner_plane']
        logger.info("nonlocal layer, inner channel size: {}".format(inner_plane))
        self.cls_z_nonlocal = NonLocalBlock(head_width, head_width, inner_plane)
        self.reg_z_nonlocal = NonLocalBlock(head_width, head_width, inner_plane)
        self.cls_x_nonlocal = NonLocalBlock(head_width, head_width, inner_plane)
        self.reg_x_nonlocal = NonLocalBlock(head_width, head_width, inner_plane)
