...
class SiamSKCorrTrack(ModuleBase):
    r"""
    ...
    def __init__(self, backbone, head, loss=None):
        super(SiamSKCorrTrack, self).__init__()
    ...
    def forward(self, *args, phase="train"):
        ...
        if phase == 'train':
            ...
            # reg sk layer
            r_out = self.sk_reg_corr(r_out)
            # cls sk layer
            c_out = self.sk_cls_corr(c_out)
            # head
    ...
    def _make_sk_conv(self):
        ...
        self.sk_reg_corr = SKConv(head_width, M=kernel_num, r=discount)
        self.sk_cls_corr = SKConv(head_width, M=kernel_num, r=discount)

    def _initialize_sk_conv(self):
        conv_weight_std = self._hyper_params['conv_weight_std']
        sk_conv_list = [
            self.sk_reg_corr, self.sk_cls_corr
        ]
